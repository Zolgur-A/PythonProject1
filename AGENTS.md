...
Руководство для людей и AI-агентов, которые вносят изменения в этот репозиторий.

## Структура проекта

- `main.py` — основной файл приложения/скрипта.
- `.venv/` — локальное виртуальное окружение Python; не изменяйте вручную и не коммитьте содержимое.
- `.idea/` — настройки PyCharm; менять только при явной необходимости.
- Новые модули размещайте в корне проекта или в отдельном пакете с понятным именем, например `app/`, если код разрастается.
- Тесты размещайте в `tests/` с именами файлов вида `test_*.py`.
...
Если появится файл зависимостей, устанавливайте их так:

```powershell
.\.venv\Scripts\python.exe -m pip install -r requirements.txt
```

Для тестов используйте `pytest`, если он добавлен в зависимости:
...
- Следуйте PEP 8: 4 пробела для отступов, понятные имена, короткие функции.
- Имена функций и переменных — `snake_case`, классов — `PascalCase`, констант — `UPPER_SNAKE_CASE`.
- Избегайте магических значений: выносите повторяющиеся строки, числа и пути в константы.
- Не дублируйте логику: если один и тот же фрагмент нужен повторно, выделите функцию.
- Добавляйте комментарии только там, где они объясняют неочевидное решение.

## Тестирование

- Для новой логики добавляйте тесты в `tests/`.
- Проверяйте граничные случаи: пустые значения, неверные типы, отсутствующие файлы, ошибки ввода.
- Тесты должны быть независимыми и не полагаться на порядок выполнения.

## Правила для агентов

- Перед изменением файла всегда прочитайте его актуальное содержимое.
- Делайте минимальные, целевые правки без переписывания несвязанного кода.
- После изменений запускайте наиболее релевантную конфигурацию или тесты, если они доступны.
- Не изменяйте `.venv/`, `.git/` и служебные файлы IDE без прямого запроса.

## Коммиты и PR

- Используйте короткие осмысленные сообщения: `Add input validation`, `Fix file parsing`.
- В PR описывайте цель изменения, основные правки и способ проверки.
...
Реализует то, что написано в спеке (спека побеждает).
Добавляет REVIEW-маркер: <!-- REVIEW: §5.3 could benefit from exponential backoff instead of fixed timeout, because [reason] -->.
Сообщает человеку в отчёте: «Я реализовал фиксированный таймаут, как в спеке, но предлагаю рассмотреть exponential backoff — см. REVIEW в PROP-001§5.3».
Человек решает в следующем цикле.
